<template>
  <div>
    <div class="header">
      <div style="color:#5e84f3;font-size: 1.2em;font-weight: bold;padding-top: 15px"><h1 style="margin: 0 0 0 20px">督察内容搜索</h1></div>
      <div style="display: flex;margin: 30px 0 0 20px">
        <div>
          所属单位：
          <el-autocomplete
            class="inline-input"
            v-model="state"
            :fetch-suggestions="querySearch"
            placeholder="请输入所属单位"
            :trigger-on-focus="false"
            @select="handleSelect"
          ></el-autocomplete>
        </div>
        <div style="margin-left:110px">
          发起人：
          <el-autocomplete
            class="inline-input"
            v-model="state"

            placeholder="请输入发起人"
            :trigger-on-focus="false"

          ></el-autocomplete>
        </div>
        <div style="margin-left:110px">
          监管人：
          <el-autocomplete
            class="inline-input"
            v-model="state"

            placeholder="请输入监管人"
            :trigger-on-focus="false"

          ></el-autocomplete>
        </div>
        <div style="margin-left:110px">
          登录时间：
          <el-date-picker
            v-model="value1"
            type="date"
            placeholder="选择日期">
          </el-date-picker>
        </div>
      </div>
      <div style="display: flex; margin:20px 35px 0 0">
        <div style="margin-left:20px">
          处理状态：
          <el-select v-model="value" clearable placeholder="请选择处理状态">
            <el-option
              v-for="item in options"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div style="margin-left: 95px">
          问题类型：
          <el-select v-model="value" clearable placeholder="请选择问题类型">
            <el-option
              v-for="item in options"
              :key="item.value"
              :label="item.label"
              :value="item.value">
            </el-option>
          </el-select>
        </div>
        <div style="margin-left: 600px">
          <el-button @click="refresh()" style="color: #5086ff;background: #e7f0fb">重置</el-button>
          <el-button @click="appQuery" style="color: #fff;background:#5086ff">查询</el-button>
        </div>
      </div>
    </div>
    <div class="footer">
      <div style="margin-left: 20px;padding-top: 15px">
        <div style="margin-bottom: 15px">
        </div>
        <div>
          <el-table
            :data="superviseList"
            border
            style="border-radius: 15px;"
            :cell-style="tableRowStyle"
            :header-cell-style="tableHeaderStyle"
            v-loading="appLoading"
            element-loading-text="加载中"
            element-loading-spinner="el-icon-loading"
            element-loading-background="rgba(0, 0, 0, 0.8)"
          >
            <el-table-column type="index" label="序号" width="90">
            </el-table-column>
            <el-table-column prop="super_type" label="所属单位">
            </el-table-column>
            <el-table-column prop="" label="发起人"> </el-table-column>
            <el-table-column prop="" label="监督人"> </el-table-column>
            <el-table-column prop="created_at" label="日期"> </el-table-column>
            <el-table-column prop="" label="处理状态"> </el-table-column>
            <el-table-column prop="" label="问题类型"> </el-table-column>
            <el-table-column prop="" label="具体问题"> </el-table-column>
          </el-table>
        </div>
        <div class="page">
          <el-pagination
            @current-change="appListChange"
            :page-sizes="[10]"
            :page-size="10"
            layout="sizes, prev, pager, next"
            :total="appTotal"
          >
          </el-pagination>
        </div></div>
    </div>
  </div>
</template>

<script>
import { contact } from '@/api/user'
import axios from 'axios'

export default {
  data () {
    return {
      state: '',
      superviseList: {
        super_type: '',
        created_at: '',
        super_content: ''
      },
      appLoading: false,
      appTotal: 0
    }
  },
  mounted () {
    this.getSuperviseList()
  },
  methods: {
    appQuery () {

    },
    refresh () {

    },
    // 表格样式
    tableRowStyle ({ row, rowIndex }) {
      if (rowIndex % 2 === 1) {
        return 'background-color: #F7F7FF;text-align:center'
      } else {
        return 'text-align:center'
      }
    },
    // 表格第一行的设置
    tableHeaderStyle ({ row, rowIndex, columnIndex }) {
      if (rowIndex === 0) {
        return 'height: 3.75rem;font-size: 1rem;font-family: Microsoft YaHei;font-weight: bold;line-height: 1.3125rem;color: #333333;text-align:center;background:#ebf5ff'
      }
    },
    async getSuperviseList () {
      const accessToken = window.localStorage.getItem('access_token')
      const Token = window.localStorage.getItem('Token')
      await axios.get('/auth/supervise/content/index', {
        headers: {
          Authorization: `Bearer ${Token}`,
          'x-api-header': 'yuanxibing',
          'x-access-token': accessToken
        }
      }).then(res => {
        console.log(res)
        this.superviseList = res.data.data.items
      })
    },
    // 分页
    async appListChange () {

    }
  }
}
</script>

<style scoped>
.header{
  width: 100%;
  height: 200px;
  background: #fff;
  border-radius: 15px;
}
.footer{
  width: 100%;
  height: 600px;
  background: #fff;
  margin-top: 20px;
  border-radius: 15px;
}
.page {
  padding: 0 1.25rem 1.25rem 1.25rem;
  text-align: right;
}
.inline-input{
  width: 217px;
}
</style>
